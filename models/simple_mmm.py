...
class SimpleMMM:
    ...
    def fit(self, spend, revenue, channels, decay_grid=np.linspace(0.1, 0.9, 20)):
        """Fit with grid search over decay rates."""
        best_score = -np.inf
        best_result = None
        n = len(revenue)
        ...
        for decay in decay_grid:
            ...
            for i in range(len(channels)):
                X[:, i+1] = adstock(spend[:, i], decay)
            
            model = LinearRegression().fit(X, revenue)
        ...
        self.model = LinearRegression().fit(best_result['X'], revenue)
    ...
    def _calc_contributions(self, spend, channels):
        ...
        for i, ch in enumerate(channels):
            self.contributions[ch] = coef[i+1] * spend[:, i].sum()
    
    def rois(self, spend, channels):
        """ROI per channel = revenue / spend."""
        rois = {}
        for i, ch in enumerate(channels):
            rev = self.params['coef'][i+1] * spend[:, i].sum()
    ...
    def summary(self, spend, channels):
        ...
        print(f"  R²: {self.params['r2']:.3f}")
        print(f"  Baseline: {self.params['intercept']:.0f}")
